"""Add governed user configuration and identity management audit events."""
...
revision: str = "0037_identity_management_operations"
down_revision: str | None = "0036_scheduled_store_reviews"
...
def upgrade() -> None:
    ...
    op.create_table(
        "identity_management_events",
        sa.Column("id", sa.String(64), primary_key=True),
        sa.Column("enterprise_id", sa.String(64), nullable=False),
        sa.Column("event_type", sa.String(80), nullable=False),
        sa.Column("target_principal_id", sa.String(64), nullable=False),
        ...
        sa.ForeignKeyConstraint(["target_account_id"], ["user_accounts.id"]),
    ...
    for column in (
        ...
        op.create_index(
            f"ix_identity_management_events_{column}",
            "identity_management_events",
            [column],
        )
...
def downgrade() -> None:
    op.drop_table("identity_management_events")
    with op.batch_alter_table("user_accounts") as batch_op:
