"""Add governed memory provider dual-run evaluation ledger."""
...
revision: str = "0028_memory_provider_evaluations"
down_revision: str | None = "0027_scoped_meeting_creation"
...
def upgrade() -> None:
    op.create_table(
        "memory_provider_evaluation_runs",
        ...
        sa.UniqueConstraint(
            "enterprise_id",
            "idempotency_key",
            name="uq_memory_provider_evaluation_enterprise_idempotency",
        ),
    )
    for column in (
        "enterprise_id",
        ...
        op.create_index(
            f"ix_memory_provider_evaluation_runs_{column}",
            "memory_provider_evaluation_runs",
            [column],
        )

    op.create_table(
        "memory_provider_evaluation_results",
        sa.Column("id", sa.String(64), primary_key=True),
        sa.Column(
            "evaluation_run_id",
            sa.String(64),
            sa.ForeignKey("memory_provider_evaluation_runs.id", ondelete="CASCADE"),
            nullable=False,
        ),
        ...
        sa.Column("status", sa.String(32), nullable=False),
        sa.Column("indexed_memory_count", sa.Integer(), nullable=False),
        ...
        sa.UniqueConstraint(
            "evaluation_run_id",
            "provider_key",
            name="uq_memory_provider_evaluation_result_provider",
        ),
    )
    for column in ("evaluation_run_id", "provider_key", "status", "created_at"):
        op.create_index(
            f"ix_memory_provider_evaluation_results_{column}",
            "memory_provider_evaluation_results",
            [column],
        )
...
def downgrade() -> None:
    op.drop_table("memory_provider_evaluation_results")
    op.drop_table("memory_provider_evaluation_runs")
